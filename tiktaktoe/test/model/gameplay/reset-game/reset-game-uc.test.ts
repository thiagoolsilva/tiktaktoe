...
import { ResetGameUseCase } from '../../../../src/model';

test('reset-game: The method initGame should be called once ', () => {
  ...
  const mockGameRepository: GameRepositoryInterface = {
    gameStatus: jest.fn(),
    initGame: jest.fn().mockReturnValue(expectedInitGameData),
    insertPlayRound: jest.fn(),
  };
  const realResetGameUC = new ResetGameUseCase(mockGameRepository);
  const initGameData = realResetGameUC.execute();

  expect(initGameData).toBe(expectedInitGameData);
